...
function NUI.SetUIVisible(state)
    ...
    Debug.Print("VISIBILITY", "SET_UI_VISIBLE -> %s (screen: ^3%s^7)", Debug.Bool(state), tostring(Storage.CurrentScreen))

    -- SendNUIMessage before the page listens is lost for good: the hud would then
    -- stay at opacity 0 with no retry, since the caller guards on UI_DataLoaded.
    if not NUI.Loaded then
        Debug.Print("VISIBILITY", "^1WARNING: NUI is not loaded yet, this SET_UI_VISIBLE will be DROPPED.^7")
        Debug.Print("VISIBILITY", "^1         The hud will never become visible. Run /hud_force_visible to confirm.^7")
    end
...
function NUI.SwitchScreen(screen)
    Debug.Print("SCREENS", "SET_SCREEN -> ^3%s^7 (NUI.Loaded=%s)", tostring(screen), Debug.Bool(NUI.Loaded))
...
function NUI.WelcomePreload()
    Debug.Print("WELCOME", "Preloading welcome flow (NUI.Loaded=%s, Storage.HasBeenSent=%s) [/]",
        Debug.Bool(NUI.Loaded), Debug.Bool(Storage.HasBeenSent))
    while not NUI.Loaded do Wait(0) end
    while not Storage.HasBeenSent do Wait(0) end
    Debug.Print("WELCOME", "Gates passed, opening the welcome screen.")
...
RegisterNUICallback("base.onBodyLoaded", function()
    Debug.Print("NUI", "^2Body loaded^7 — from now on SendNUIMessage reaches the page.")
    NUI.Loaded = true
