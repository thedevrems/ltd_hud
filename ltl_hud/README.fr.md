...
| `ox_lib` | amorçage partagé (`@ox_lib/init.lua`) et callbacks serveur |
| `oxmysql` | table `ltl_hud_settings` : le HUD de chaque compte |
...
sont enregistrées en base dans la table `ltl_hud_settings`, indexée par l'identifiant du
compte — la licence, pas le personnage. Un joueur retrouve donc la même interface sur
chacun de ses persos, et sur n'importe quelle machine. La table est créée au démarrage
par `server/components/settings.lua` ; `ltl_hud.sql` permet de l'appliquer à la main.

Chaque modification part vers le serveur immédiatement ; les écritures sont regroupées
sur 750 ms pour qu'un curseur qu'on fait glisser ne produise pas une requête MySQL par
pixel. La commande `/remove_nui_storage` supprime la ligne du compte.
...
| `storage.js` | document des réglages du compte, semé par Lua depuis `ltl_hud_settings` |
...
| `config.js`, `bootstrap.js` | fusion de la config Lua et des réglages en base, publication des variables CSS |
...
L'interface répond avec **34** callbacks, tous de la même forme (les deux derniers,
`storage.onMusicUpdate` et `storage.setConfigured`, portent ce qui s'écrivait auparavant
directement dans le `localStorage`) :
