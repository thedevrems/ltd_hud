...
Players.SetData = function(src)
    ...
    -- `id` reste l'identifiant serveur, que d'autres ressources lisent depuis le
    -- statebag UI_UserData. Ce n'est plus lui que l'interface affiche : elle
    -- montre l'UUID, le code public à quatre caractères de ltl_core, qui désigne
    -- le personnage d'une session à l'autre là où l'id serveur change à chaque
    -- connexion et ne veut rien dire pour le joueur d'en face.
    data.id = src
    data.uuid = xPlayer.getUUID()
...
-- ltl_core émet les deux évènements en local (server/main.lua) : pas de RegisterNetEvent,
-- sinon un client pourrait déclencher le chargement ou le déchargement d'un autre joueur.
AddEventHandler('ltl:playerLoaded', function(src, _, _)
    Players.SetData(src)
    TriggerClientEvent('ltl_hud:Client:LoadPlayer', src)
end)

AddEventHandler('ltl:playerLogout', function(src)
