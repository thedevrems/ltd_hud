...
Built for the LTL Framework (`ltl_core`) and pma-voice.
...
| Dependency | Purpose |
| --- | --- |
| `ltl_core` | player data, job, money |
| `pma-voice` | voice indicator and proximity |
| `ox_lib` | shared bootstrap (`@ox_lib/init.lua`) and server callbacks |
| `oxmysql` | the `ltl_hud_settings` table: one HUD per account |
| `ltl_status` / `esx_status` | *optional* — feeds the hunger, thirst and stress gauges |
...
The resource reads the framework through `exports['ltl_core']:getSharedObject()` and
listens to `ltl:playerLoaded`, `ltl:playerLogout`, `ltl:setJob` and the three
`ltl:*AccountMoney` events.

Statuses live outside the core. `client/addon/status.lua` subscribes to **both**
`ltl_status:onTick` and `esx_status:onTick`, so the hunger, thirst and stress gauges keep
working through the `esx_status` → `ltl_status` rename with no edit on this side. Once the
rename has landed everywhere, drop `esx_status` from the `StatusProviders` list at the top
of that file.
...
Player overrides (chosen variants, colours, component positions, presets) are stored in the
`ltl_hud_settings` table, keyed by the account identifier — the licence, not the character.
A player therefore keeps one interface across every character of theirs, and across
machines. The table is created on boot by `server/components/settings.lua`; `ltl_hud.sql`
is there for operators who apply their schema by hand.

Every change is sent to the server as it happens; writes are coalesced over 750 ms so
dragging a slider does not produce one MySQL query per pixel. `/remove_nui_storage` drops
the account's row.
...
| `storage.js` | the account settings document, seeded by Lua from `ltl_hud_settings` |
...
| `config.js`, `bootstrap.js` | merge the Lua config with the stored settings, publish CSS variables |
...
The interface answers with **34** callbacks, all sharing one shape (the two most recent,
`storage.onMusicUpdate` and `storage.setConfigured`, carry what used to be written straight
into `localStorage`):
