...
// Un serveur bien fourni enregistre plusieurs centaines de commandes : au-delà
// d'une poignée, la liste couvre l'écran au lieu de le renseigner.
function suggestionLimit() {
    const value = config.state.Chat && config.state.Chat.SuggestionLimit;
    return Number.isFinite(value) && value > 0 ? value : DEFAULT_SUGGESTIONS;
}

// Shortest command first, then alphabetically, capped by the config.
