...
//
// Switching faster than the .8s leave leaves several previews in the box at
// once. Reading only the first child then re-animates that same one on every
// swap and strands the ones behind it, so every child still present has to be
// sent out, and only once.
export function swapPreview(preview, node) {
    Array.from(preview.children).forEach(previous => {
        if (previous.dataset.leaving) return;
        previous.dataset.leaving = "1";
        leaveTransition(previous, PREVIEW_KIT, () => previous.remove());
    });
    if (!node) return;
