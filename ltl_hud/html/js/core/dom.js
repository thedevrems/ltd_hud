...
// Strips whatever leave phase is still applied, whichever kit started it. The
// pending timer will try to remove these again later, which is a no-op.
export function clearLeaveTransition(node) {
    Array.from(node.classList)
        .filter(name => /-leave-(active|from|to)$/.test(name))
