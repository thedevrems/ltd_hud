...
RegisterNUICallback("storage.switchedScreen", function(data)
    local prevScreen = Storage.CurrentScreen
    Storage.OnScreenSwitched(data.path)
    Storage.CurrentScreen = data.path

    if prevScreen == data.path then return end
    debugPrint("[^3SCREENS^7] Screen has been switched to: [^3" .. data.path:upper() .. "^7]")
end)

RegisterNUICallback("storage.setCreatedUI", function()
...
RegisterNUICallback("storage.onLoad", function(data)
    debugPrint("[^5CB^7] Storage preloaded")
    Storage.Data = data
