...
local function startProgress(data)
    ...
    if progress == false or duration <= data.duration then
        exports['ltl_hud']:CancelProgressBar()
        return false
    end

    exports['ltl_hud']:FinishProgressBar()
...
function lib.progressBar(data)
    ...
    if not interruptProgress(data) then
        exports['ltl_hud']:ProgressBar('fas fa-circle-notch', data.label, data.duration, nil, nil, data.canCancel, nil, nil, nil, nil, true)
...
RegisterNetEvent('ltl_hud:ProgressComplete', function()
    progress = nil
end)
