AddEventHandler('onClientResourceStart', function(resource)
	...
	-- Defaults to off here, unlike upstream: ltl_hud already draws the range, so
	-- the built-in bottom right readout would only say the same thing twice.
	-- `setr voice_enableUi 1` brings it back for a server without such a hud.
	sendUIMessage({
